...
export default function EquiposPage() {
  ...
  async function getEquipos() {
    // Traemos los equipos activos y hacemos un "join" con la tabla ubicaciones
    ...
    <div className="animate-fadeIn text-gray-900 p-4 max-w-7xl mx-auto">
      ...
      <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6 flex flex-col md:flex-row gap-4">
        <div className="flex-1">
          ...
          <input 
            type="text" 
            placeholder="Ej: CPU-001, Dell, Optiplex..." 
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full border border-gray-300 rounded-md p-2 text-sm text-gray-900 bg-white shadow-sm"
          />
        </div>
        ...
        <table className="min-w-full divide-y divide-gray-200">
          <thead className="bg-gray-50">
            <tr>
              ...
              <th className="px-4 py-4 text-left text-xs font-bold text-gray-500 uppercase">Ubicación</th>
              <th className="px-4 py-4 text-center text-xs font-bold text-gray-500 uppercase">Estado Técnico</th>
              <th className="px-4 py-4 text-center text-xs font-bold text-gray-500 uppercase">Acciones</th>
            </tr>
          </thead>
          <tbody className="bg-white divide-y divide-gray-100">
            {loading ? (
              <tr><td colSpan={6} className="text-center py-12 text-gray-400">Cargando base de datos...</td></tr>
            ) : equiposFiltrados.length === 0 ? (
              <tr><td colSpan={6} className="text-center py-12 text-gray-500">No se encontraron equipos con esos filtros.</td></tr>
            ) : (
              equiposFiltrados.map((equipo) => (
                <tr key={equipo.id_equipo} className="hover:bg-gray-50 transition-colors">
                  <td className="px-4 py-4 whitespace-nowrap text-sm font-bold text-blue-700">{equipo.cod_patrimonio}</td>
                  ...
                  <td className="px-4 py-4 whitespace-nowrap text-center text-sm font-medium space-x-2">
                    <Link 
                      href={`/detalles-equipo/${equipo.id_equipo}`} 
                    ...
                    <Link 
                      href={`/editar-equipo/${equipo.id_equipo}`} 
                      className="inline-flex items-center justify-center w-8 h-8 rounded-md bg-amber-50 text-amber-600 hover:bg-amber-100 transition-colors"
                      title="Editar Equipo"
                    >
                      ⚙️
                    </Link>
                    <button 
                      onClick={() => handleDelete(equipo.id_equipo, equipo.cod_patrimonio)}
